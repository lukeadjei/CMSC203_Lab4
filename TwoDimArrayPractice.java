...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    //  and column is the index of the array's current column
    // Write your code here:
	   for (int row1 = 0; row1 < intArray.length; row1 ++) {
		   for (int col = 0; col < intArray[row1].length; col++) {
			   if (row1 == row) {
				   intArray[row1][col] = value;
			   }
			   animate( row, col);
		   }  
    ...
    // Write your code here:
	   ...
	   for (int row = 0; row < intArray.length; row ++) {
		   for (int col = 0; col < intArray[row].length; col++) {
				  if (col == column) {
					  if (row == 0) {
						  minimum = intArray[row][col];
					  }
					  else {
						  if (intArray[row][col] < minimum) {
							  minimum = intArray[row][col];
